...
export default Ember.Component.extend({	
	...
	itemsView: Ember.CollectionView.extend({		
		...
		itemViewClass: Ember.View.extend({
			classNames: ['item'],
			classNameBindings: ['content.isActive:active'],
			template: Ember.Handlebars.compile('{{#light-box href=view.content.medium data-lightbox="test" data-title="title" inlineImage=false}}<img {{bind-attr src="view.content.medium"}} alt=""/>{{/light-box}}<div class="carousel-caption"><h4>{{view.content.title}}</h4><p>{{view.content.content}}</p></div>')
		})
	})
});
